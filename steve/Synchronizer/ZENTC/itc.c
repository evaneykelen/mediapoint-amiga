...
int __asm ITCIntHandler( register __a1 SYNCDATA *SD)
{
  ...
  BOOL B_SendSignal;
	...
	// if set then this interrupt will generate framecodes
	// else the FCFastCheck should be set by another part of the
	// program. As done with Zentc in which the frame is set in the main
	// loop of the synchronizer. (RepP_StoZen)
	if(SD->sd_GenerateInternally)
