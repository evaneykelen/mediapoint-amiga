...
extern ULONG		SigR_StoZen,	// signal from Zen
...
extern struct MsgPort
					*Port_StoZen,	// port through which the synchro may talk to zen
...
extern TIMEREQUEST	*Msg_Timer,
					*Msg_RTimer;
extern struct TCMsg	 Msg_ZenDial;	// message to Zen

extern BOOL			RemoveOK, 
					TermOK;
extern BOOL			B_PunchReceived,
					B_ZenReceived;
...
void TempoEditor( void)
{
  ...
  ULONG CurFC;
	...
	if( 
		...
		((RepP_StoZen = CreatePort(NULL,0)) == NULL) ||
	...
	while(!TermOK)
	{
		SigRecvd = Wait(SigR_StoZen|Sig_TCItoS|Sig_PCtoS|SigR_StoPC|SEventData.ed_Sig_ItoS|SIGF_ABORT);
...
#if _PRINTF
								printf("from pc: syncpunch type = %d\n",SyncPunch->sp_PunchType);
								printf("from pc: Msg_Timer at %x\n",(int)Msg_Timer);
#endif
								if(RemoveOK)
...
#if _PRINTF
						printf("Returning punch %d\n",Msg_Timer->tr_Punch->sp_PunchType);
#endif
						ReturnPunch(SGList,Msg_Timer,RepP_StoPC,Port_StoPC);
					}
				}
