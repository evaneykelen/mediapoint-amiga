...
#define XAPPPORTNAME "Port_MPMusic"
...
void main( int argc, char *argv[] )
{
	...
	{
		B_Setup = SetupPlayer( interrupt_offset );
		if (B_Setup)
			return;
...
//KPrintF("modtype = %d\n", modType);
	...
	if( ThisPI->pi_Arguments.ar_Worker.aw_NumArgs[1] == 1 )
		B_Stopper = TRUE;
	else
		B_Stopper = FALSE;
	...
	interrupt_offset = ( (MLSystem->refreshRate+1)<<4 )/50 ;

//	KPrintF("Setting offset at %ld\n",interrupt_offset );
	...
	if( !B_Stopper && !VARFile && ( MLSystem->miscFlags & 1L << 6 ) )
	{
		//KPrintF("Reading module 1\n");
	...
	while(!B_Term)
	{
		...
		if(SigRecvd & SigR_XtoX)
		{
			...
			{
				//KPrintF("Other is told to stop playing\n");
				B_Run = FALSE;
			}
		}
		...
		else
		{
			if(B_Run)
			{
				if( VARFile )
				{
					...
					//KPrintF("Music Found 2 var [%s]\n",Varcont );

					if( strcmp( FilePath, Varcont ) )		// are they different ?
					{
						if( MusicModule != NULL )
						{
							if( B_Playing )
				...
				if( MusicModule != NULL )
				{
					...
					{
						if((B_Setup = !SetupPlayer( interrupt_offset )))
						{
							if(!(B_Playing = !PlayTune(MusicModule)))
							{	// couldn't play tune, remove player and module from memory
								KillModule(MusicModule);
