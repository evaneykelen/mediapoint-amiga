...
;	bsr	create_tables
	...
	move.l	im_width(a5),d0	
	add.l	d0,d0			; words
	move.l	d0,dith_linesize(a0)
	subq.l	#2,dith_linesize(a0)
	addq.l	#2,d0			; prevent pixel wrap
	move.l	d0,dith_modulo(a0)
	move.l	d0,d5
	lea	bufdata(pc),a1
	addq.l	#4,a1
	move.l	a1,dith_red_start(a0)
	add.l	d0,a1
	move.l	a1,dith_green_start(a0)
	add.l	d0,a1
	move.l	a1,dith_blue_start(a0)
	add.l	d0,a1
	...
	move.l	d0,dith_linesize(a0)
	subq.l	#2,dith_linesize(a0)
	bsr	restart_dith_buf
	lea	bufdith1(pc),a0
	bsr	restart_dith_buf
	rts

restart_dith_buf:
	move.l	dith_red_start(a0),dith_red(a0)
	move.l	dith_green_start(a0),dith_green(a0)
	move.l	dith_blue_start(a0),dith_blue(a0)
	moveq	#0,d0
	move.l	dith_red_start(a0),a2
	move.l	d0,-4(a2)
	move.w	d0,(a2)
	move.l	dith_green_start(a0),a2
	move.l	d0,-4(a2)
	move.w	d0,(a2)
	move.l	dith_blue_start(a0),a2
	move.l	d0,-4(a2)
	move.w	d0,(a2)
	rts
...
convert_pens_dith_line:
	...
	move.l	dith_red(a0),a0		; a0 points to red errors

	move.l	im_width(a5),d6
	subq.l	#1,d6			; number off pixels
.rep_rgb:
	move.l	im_width(a5),d7
	move.l	dith_modulo(a6),d5
	moveq	#0,d0
	moveq	#0,d1
	moveq	#0,d2
...
convert_pens_dith_line2:
	...
	move.l	buf_point(pc),a0
	move.l	buf_point2(pc),a6	; a6 points to second line
	move.l	dith_linesize(a0),d6

	add.l	d6,dith_red(a0)		; set error buffers at the end
	add.l	d6,dith_green(a0)
	add.l	d6,dith_blue(a0)
	add.l	d6,dith_red(a6)
	add.l	d6,dith_green(a6)
	add.l	d6,dith_blue(a6)
...
floyd_store:
	...
	move.l	dith_red(a6),a2
	bsr	floyd_pen_store
	move.l	dith_modulo(a6),d7
	move.w	diffs+2(pc),d0		; green difference
	move.l	dith_green(a6),a2
	bsr	floyd_pen_store
	move.w	diffs+4(pc),d0		; blue difference
	move.l	dith_blue(a6),a2
	add.l	d7,d7
	bsr	floyd_pen_store
	addq.l	#2,dith_red(a6)
	addq.l	#2,dith_green(a6)
	addq.l	#2,dith_blue(a6)
	rts	
...
floyd_store2:
	...
	add.l	d7,d7
	bsr	floyd_pen_store2
	subq.l	#2,dith_red(a6)
	subq.l	#2,dith_green(a6)
	subq.l	#2,dith_blue(a6)
	rts	
...
burk_convert_pens_dith:
	...
	move.l	im_height(a5),d5
.rep_rgb2:
	bsr	burk_convert_pens_dith_line
	add.l	a6,a4			; source modulo
	subq.w	#1,d5
	beq	.exit

	bsr	burk_convert_pens_dith_line2
	add.l	a6,a4			; source modulo
	subq.w	#1,d5
	beq	.exit
...
bufdata:	ds.w	12000		; room for width's upto 1000 pixels
	...
	RSRESET
dith_struct:		rs.w	0
dith_red_start:		rs.l	1	; points to red errors ( words )
dith_green_start:	rs.l	1
dith_blue_start:	rs.l	1
dith_red:		rs.l	1	; points to red errors ( words )
dith_green:		rs.l	1
dith_blue:		rs.l	1
dith_modulo:		rs.l	1
