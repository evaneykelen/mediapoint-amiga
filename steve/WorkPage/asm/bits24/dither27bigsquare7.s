...
;	dc.w	17,102,153,0		; 0
;	dc.w	0,17,17,0		; 1
;	dc.w	255,238,221,0		; 2
;	dc.w	170,153,204,0		; 3
;	dc.w	85,85,85,0		; 4
;	dc.w	0,255,0,0		; 5
;	dc.w	0,0,255,0		; 6
;	dc.w	255,0,0,0		; 7
...
create_lookup_table:
	...
	move.l	a0,a1
	add.l	#4*4096,a1	; point to first free area
	moveq	#8,d0		; required space for color table
...
store_surround:
	...
	sub.w	#16,d0			; store prev red
	bsr	store_in_table
	movem.w	(a7),d0/d1/d2

	sub.w	#16,d0			; prev red
	sub.w	#16,d1			; prev green
	bsr	store_in_table
	movem.w	(a7),d0/d1/d2

	sub.w	#16,d0			; prev red
	sub.w	#16,d2			; prev blue
	bsr	store_in_table
	movem.w	(a7),d0/d1/d2
...
.rep_tt2:
	...
	addq.b	#1,(a1)		; add one for easy search test
	dbf	d7,.rep_tt2
	rts
...
store_in_table:
	...
	cmp.b	#7,d5
	beq	.no_store		; no more than 7 entries
.do_search:
...
;	movem.l	d0/d1/d2,-(a7)
	...
	movem.l	(a7)+,d0/d1/d2
	move.b	d4,d5
	movem.l	d0/d1/d2/d5,-(a7)
	bsr	find_best_col
	movem.l	(a7)+,d0/d1/d2/d5
	cmp.b	d4,d5
	beq	.okie
	addq.l	#1,teller
	nop
...
; done line now switch buffers
	...
	rts
*
* a0 points to the row of the ordered matrix
*
convert_pens_ordered:
	movem.l	a6/d5,-(a7)
...
convert_pens_ord:
	...
	move.l	im_height(a5),d5
.rep_rgb2:
	lea	ordered_matrix(pc),a0
	move.l	d5,d0
	and.l	#$3,d0
	lsl.l	#3,d0
	add.l	d0,a0
	bsr	convert_pens_ordered
	add.l	a6,a4			; source modulo
	subq.w	#1,d5
	beq	.exit
	btst	#6,$bfe001
...
ltable:		blk.b	50000,0		; in the program for now
