			...
			*Dest;
{
  int len1, len2;
	...
	if(Src1->vc_Type == VCT_STRING)
	{
		...
		if(Src2->vc_Type == VCT_STRING)
		{
			len1 = strlen( Src1->vc_Value.vv_String );
			len2 = strlen( Src2->vc_Value.vv_String );
			if ( len1-len2 >= 0 )
				Dest->vc_Value.vv_String[ len1-len2 ] = '\0';
			Dest->vc_Type = VCT_STRING;
		}
	}
	else
...
struct Library *medialinkLibBase;
	...
	if ( vcr )
	{
		...
		if ( medialinkLibBase )
		{
			UA_IssueRexxCmd_V2("MP_VARS","MEDIAPOINT",cmd,NULL,NULL);
