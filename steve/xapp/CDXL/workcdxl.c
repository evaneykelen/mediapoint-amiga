...
void main( int argc, char *argv[] )
{
  ...
  BOOL			B_CDxlHasRun,
	...
	{
		if( (CDxlDataSegment = (UBYTE *)MLMMU_AllocMem(get_varsize(), MEMF_PUBLIC|MEMF_CLEAR,NULL)) != NULL)
		{
								...
								if(ErrLoadCDxl)
								{
									if(MLMMU_AvailMem(MEMF_PUBLIC) > 50000)
									{
										if(CDxlDataSegment == NULL)
											CDxlDataSegment = (UBYTE *)MLMMU_AllocMem(get_varsize(), MEMF_PUBLIC|MEMF_CLEAR,NULL);
