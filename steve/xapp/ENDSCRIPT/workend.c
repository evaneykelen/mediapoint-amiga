	...
	if( DOSBase )
	{
		...
		if( lock )
		{
			...
			if( run )
			{
				...
				sprintf(tt,"assign ALIAS: mp:script%d\n",run );
				Execute( tt,0,0 );							// do  alias

// Delete and copy all files from script1 to script2

				if( run == 1 )
					strcpy(tt,"MP:script2");
				else
					strcpy(tt,"MP:script1");
