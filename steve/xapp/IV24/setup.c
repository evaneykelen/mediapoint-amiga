...
struct Task *oldTask;
	...
	UA_TranslateGR(FYE_GR, msgs);
	UA_TranslateGR(A1_GR, msgs);
	UA_TranslateGR(A2_GR, msgs);
	UA_TranslateGR(A3_GR, msgs);
	UA_TranslateGR(A4_GR, msgs);
	UA_TranslateGR(A5_GR, msgs);
	...
	while(loop)
	{
		...
		if (CED.Class==MOUSEBUTTONS && CED.Code==SELECTDOWN)
		{
			...
			else if ( IV24.action==ACTION_FB )
			{
				...
				ProcessPage3(window, ID, &IV24, &CED, mypattern1);
			}
			...
			switch(ID)
			{
				case 4:		// Preview
					...
					if ( !XappDoIt(ThisPI,&IV24) )
						GiveMessage(window, msgs[Msg_IV_MISC_1-1]); // "The IV-24 doesn't respond."
...
TEXT temp[SIZE_FULLPATH];
	...
	else if ( IV24->action==ACTION_FB )
	{
		if ( IV24->fileName[0] != '\0' )
		{
			...
			UA_ShortenStringFront(window->RPort, temp, (A3_GR[0].x2-A3_GR[0].x1)-16);
...
BOOL touched=FALSE;
	...
	if (touched)	// sigh...got to drag info from cycles, strings etc.
	{
		...
		if (IV24->pip_x<30 || IV24->pip_x>500)
		{
			IV24->pip_x = 40;
			UA_SetStringGadgetToVal(window, &A1_GR[5], IV24->pip_x);
			GiveMessage(window, msgs[Msg_X_4-1], 30, 500); // "Enter a value between 30 and 500."
		}
		...
		if (IV24->pip_y<28 || IV24->pip_y>605)
		{
			IV24->pip_y = 40;
			UA_SetStringGadgetToVal(window, &A1_GR[6], IV24->pip_y);
			GiveMessage(window, msgs[Msg_X_4-1], 28, 605); // "Enter a value between 28 and 605."
		}
...
TEXT temp[SIZE_FULLPATH];
	...
	switch(ID)
	{
		case 0:	// file
			...
			if ( PickPicture(window,path,mypattern1) )
			{
	...
	switch(ID)
	{
		...
		case 1:
			...
			if (IV24->comp_from<0 || IV24->comp_from>255)
			{
				...
				GiveMessage(window, msgs[Msg_X_4-1], 0, 255);	// "Enter a value between 0 and 255."
			}
			break;
...
void DisplayModeInfo(struct Window *window, struct IV24_actions *IV24)
{
	...
	if ( IV24->action == ACTION_PIP )
	{
		...
		{
			UA_DrawSpecialGadgetText(window, &FYE_GR[11], msgs[Msg_IV_PM_1-1], SPECIAL_TEXT_CENTER);
			UA_DrawSpecialGadgetText(window, &FYE_GR[12], msgs[Msg_IV_PM_2-1], SPECIAL_TEXT_CENTER);
		}
		else if ( IV24->pip_display_mode==1 )
		{
			UA_DrawSpecialGadgetText(window, &FYE_GR[11], msgs[Msg_IV_PM_3-1], SPECIAL_TEXT_CENTER);
			UA_DrawSpecialGadgetText(window, &FYE_GR[12], msgs[Msg_IV_PM_4-1], SPECIAL_TEXT_CENTER);
		}
		else if ( IV24->pip_display_mode==2 )
		{
			UA_DrawSpecialGadgetText(window, &FYE_GR[11], msgs[Msg_IV_PM_5-1], SPECIAL_TEXT_CENTER);
			UA_DrawSpecialGadgetText(window, &FYE_GR[12], msgs[Msg_IV_PM_6-1], SPECIAL_TEXT_CENTER);
		}
		else if ( IV24->pip_display_mode==3 )
		{
			UA_DrawSpecialGadgetText(window, &FYE_GR[11], msgs[Msg_IV_PM_7-1], SPECIAL_TEXT_CENTER);
			UA_DrawSpecialGadgetText(window, &FYE_GR[12], msgs[Msg_IV_PM_8-1], SPECIAL_TEXT_CENTER);
		}
		else if ( IV24->pip_display_mode==4 )
		{
			UA_DrawSpecialGadgetText(window, &FYE_GR[11], msgs[Msg_IV_PM_9-1], SPECIAL_TEXT_CENTER);
			UA_DrawSpecialGadgetText(window, &FYE_GR[12], msgs[Msg_IV_PM_10-1], SPECIAL_TEXT_CENTER);
		}
		else if ( IV24->pip_display_mode==5 )
		{
			UA_DrawSpecialGadgetText(window, &FYE_GR[11], msgs[Msg_IV_PM_11-1], SPECIAL_TEXT_CENTER);
			UA_DrawSpecialGadgetText(window, &FYE_GR[12], msgs[Msg_IV_PM_12-1], SPECIAL_TEXT_CENTER);
		}
	}
	else if ( IV24->action == ACTION_FSV )
	{
...
ULONG	error;
	...
	{
		GiveMessage(window, msgs[Msg_IV_MISC_2-1]); // "The fye.library is not available."
		return(FALSE);
	}

	if (error=ObtainFyeBoard())
	{
		GiveMessage(window, msgs[Msg_IV_MISC_3-1]); // "The IV-24 card is not available."
