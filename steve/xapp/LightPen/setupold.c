...
long size;
	...
	if ( ThisPI->pi_Arguments.ar_Worker.aw_ExtraData[0] != '\0' )
	{
		...
		UA_ShortenString(window->RPort, temp, (Light_GR[2].x2-Light_GR[2].x1)-16);
		UA_DrawText(window, &Light_GR[2], temp);
		UA_SetCycleGadgetToVal(window,&Light_GR[6],mode);
		size = GetFileSize(fullpath);
		sprintf(temp,"%d",size);
		MakeNiceNumber(temp,sizestr);
//		UA_DrawText(window, &Light_GR[5], sizestr);
	}
//	else
//		UA_DrawText(window, &Light_GR[2], msgs[Msg_X_5-1]);

	UA_SetStringGadgetToVal(window, &Light_GR[2], 30 );
	UA_SetStringGadgetToVal(window, &Light_GR[3], 0 );
