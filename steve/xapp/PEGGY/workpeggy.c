...
//	Uses		:
//	Date		:	24 sept 1994
//	Author	:	S. Vanderhorst ( Music XaPP ) adapted Peggy XaPP C.Lieshout
//	Desc.		:	Play a MPEG file on the Peggy board
//
...
#define PEGPORTNAME "Port_MLPeggy"
...
void main( int argc, char *argv[] )
{
	...
	PROCDIALOGUE	*Msg_XappDial,		// Dialogue to be send to an already active Xapp
						...
						*Msg_PegDial,		// Our dialogue 
						*Msg_RPegDial;	// Dialogue when our guide replies
	...
	struct MsgPort	*Port_Xapp,			// Inter-Xapp communication receive port
						...
						*RepP_WorkPeg;	// Reply port for our parent when 
...
//		init_mpeg( mp, Filename );
	...
	if( (RepP_WorkPeg = (struct MsgPort *)CreatePort(0,0)) == NULL)
	{
	...
	// Make a Dialogue 
	if( (Msg_PegDial = (PROCDIALOGUE *)MLMMU_AllocMem(sizeof(PROCDIALOGUE), 
														MEMF_PUBLIC|MEMF_CLEAR, NULL )) == NULL)
	{
		CloseLibrary(MLMMULibBase);
		DeletePort(RepP_WorkPeg);
		return;
	}

	// Set up the Dialogue message
	Msg_PegDial->pd_Msg.mn_Node.ln_Type = NT_MESSAGE;
	Msg_PegDial->pd_Msg.mn_Length = sizeof(PROCDIALOGUE);
	Msg_PegDial->pd_Msg.mn_ReplyPort = RepP_WorkPeg;
	Msg_PegDial->pd_ChildPI = ThisPI;
	Msg_PegDial->pd_InUse = TRUE;
	Msg_PegDial->pd_Cmd = DCI_CHILDREADY;

	SigR_CtoP  		=   1 << RepP_WorkPeg->mp_SigBit;
	Sig_PtoC     	=   1 << ThisPI->pi_Port_PtoC->mp_SigBit;
	PutMsg(ThisPI->pi_Port_CtoP,(struct Message *)Msg_PegDial);

	// Make A dialogue to send to other Xapps  
	if( (Msg_XappDial = (PROCDIALOGUE *)MLMMU_AllocMem(sizeof(PROCDIALOGUE), 
														MEMF_PUBLIC|MEMF_CLEAR, NULL )) == NULL)
	{
		MLMMU_FreeMem( Msg_PegDial );
		CloseLibrary(MLMMULibBase);
		DeletePort(RepP_WorkPeg);
		return;
	}
...
//	if( action != 2 )
//		PegErr = read_and_evaluate( FileName, &smfi );
...
//		printf("Signal %x\n",SigRecvd );
		if(SigRecvd & Sig_PtoC)
		{
			...
			while( (Msg_RPegDial = (PROCDIALOGUE *)GetMsg(ThisPI->pi_Port_PtoC)) != NULL)
			{
				Msg_RPegDial->pd_ChildPI = ThisPI;	
				switch(Msg_RPegDial->pd_Cmd)
				{
					case DCC_DOPREPARE:
	//printf("DOprep\n");
						Msg_RPegDial->pd_Cmd = DCI_CHILDPREPARES;                 
						B_ReInit = TRUE;
						break;                          
					case DCC_DORUN:
	//printf("DO_run\n");
						...
						Forbid();
						if( (Port_OtherXapp = (struct MsgPort *)FindPort(PEGPORTNAME)) != NULL)
						{
...
//								printf("Send stop to other xapp\n");
						...
						else
						{
							...
							{
							   Port_Xapp = (struct MsgPort *)CreatePort(PEGPORTNAME,0);
							   Sig_XtoX = 1 << Port_Xapp->mp_SigBit;
								B_Run = TRUE;
							}
						}
						Permit();
						Msg_RPegDial->pd_Cmd = DCI_CHILDRUNS;                 
						break;                          
					case DCC_DOHOLD:
						Msg_RPegDial->pd_Cmd = DCI_CHILDHOLDS;                 
						break;
					case DCC_DOTERM:
						Msg_RPegDial->pd_Cmd = DCI_CHILDTERM;
						B_Remove = TRUE;
						B_Stop = TRUE;
						B_Run = FALSE;
						B_ReInit = FALSE;
						break;
					case DCC_DOEASYTERM:
						Msg_RPegDial->pd_Cmd = DCI_CHILDEASYTERM;
						B_Remove = TRUE;
						B_Stop = TRUE;
						B_Run = FALSE;
						B_ReInit = FALSE;
						break;
					case DCC_DOSTOP:
//						Msg_RPegDial->pd_Cmd = DCI_CHILDREADY;
						Msg_RPegDial->pd_Cmd = DCI_IGNORE;
//						B_Stop = TRUE;
//						B_Run = FALSE;
//						B_ReInit = FALSE;
						break;
					case DCC_DOEASYSTOP:
						Msg_RPegDial->pd_Cmd = DCI_CHILDEASYSTOP;
						B_Stop = TRUE;
						B_Run = FALSE;
						B_ReInit = FALSE;
						break;
				...
				ReplyMsg((struct Message *)Msg_RPegDial);
			}
		}

		// get a reply from our guide
		if(SigRecvd & SigR_CtoP)
			while( (Msg_RPegDial = (PROCDIALOGUE *)GetMsg(RepP_WorkPeg)) != NULL)
			{
				Msg_RPegDial->pd_InUse = FALSE;
				if(Msg_RPegDial->pd_Cmd == DCC_DOTERM)
					B_Remove = TRUE;
			}
		...
		if(SigRecvd & SigR_XtoX)
		{
			...
			while( (Msg_RPegDial = (PROCDIALOGUE *)GetMsg(RepP_Xapp)) != NULL);
			if( action != 2 )		// not stop other
			{
				if( (Port_Xapp = (struct MsgPort *)CreatePort(PEGPORTNAME,0)) != NULL)
				{
...
//					PegErr = read_and_evaluate( FileName, &smfi );
				...
				if( PegErr == 0 )
				{
					mp->action = ( WORD )action;
					mp->mport_ptoc = ThisPI->pi_Port_PtoC;
					mp->sig_xtox = Sig_XtoX;
					mp->sig_ptoc = Sig_PtoC;
					mp->mainsignal = Sig_XtoX | Sig_PtoC;
