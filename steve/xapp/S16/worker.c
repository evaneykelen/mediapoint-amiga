...
extern int XappDoIt(PROCESSINFO *ThisPI, struct Studio_record *Studio_rec, struct ActiveSample *asList[]);
...
#define XAPPPORTNAME "Studio16XaPP"
...
*/
	...
	while(!B_Term)
    {
		...
		if(B_Run)
		{
			XappDoIt(ThisPI,&Studio_rec,asList);
			B_Run = FALSE;
		}
