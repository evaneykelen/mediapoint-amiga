...
char tmp[USERAPPLIC_MEMSIZE], scrStr[USERAPPLIC_MEMSIZE];
	...
	/** 1 "file" loops vol freq fadeIn balance fromDisk track filter soundBox **/
	/** 2 track																								       					**/
	/** 3 secs track																					      					**/ 
	/** 4 secs track																					      					**/ 
	/** 5 vol track																						       					**/ 
	...
	sample_rec->soundBox = FALSE;	// ensure backwards compatibility - soundBox is off by default
...
//KPrintF("GVFPI [%s]\n",sample_rec->filename);
				...
				case 10: // 11th arg
					sscanf(tmp, "%d", &(sample_rec->soundBox));
					break;
...
//KPrintF("PVTPI [%s]\n",sample_rec->filename);
	...
	if ( sample_rec->filename[0]!='@' )
	{
		...
		strcpy(path2,sample_rec->filename);
