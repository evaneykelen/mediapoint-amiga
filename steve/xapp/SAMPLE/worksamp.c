...
void main( int argc, char *argv[] )
{
	...
	strcpy( Varname, sample_rec.filename );
	...
	if( sample_rec.action == SAMPLE_PLAY && !VARFile)
	{
...
KPrintF("Set bterm to False\n");
		...
		else
		{
			if( B_Run )
			{
				...
				if( VARFile )
				{
					FindVarContents( Varname, MLSystem->VIList, Varcont );
					if( strcmp( sample_rec.filename, Varcont ) )
					{
						strcpy( sample_rec.filename, Varcont );
						VARFound = TRUE;
					}
				}

				if( ( lerr && B_Playing ) || ( VARFound && lerr && B_Playing ) )	// remove the sample played now
				{
					exit_sound( &si );
					SetTaskPri( task,oldpri );
					reset_sound( &si );
				...
				if( lerr && VARFound)
				{
					freesound( &si );			// free old file load new
					lerr = loadsoundfile( &si, sample_rec.filename, TRUE, 0L );
				}
