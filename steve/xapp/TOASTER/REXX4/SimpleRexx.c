/*
 *
 * Toolmaker Simple ARexx interface... (SAS version)
 *
 ...
 *
 * Appends .1, .2, etc. to requested portname as specified in Style Guide
 *
*/
 ...
 * If you use at least 2.04 amiga.lib, this is not needed...
*/
...
short SendARexxMsg(AREXXCONTEXT RexxContext,char *RString,short StringFile, struct FileHandle *FH)
  {
  ...
  if ((RexxContext) && (RString))
	...
	if ( FH )
	  ...
	  rmsg->rm_Stdout = (LONG)FH;
        ...
        /* We need to find the RexxPort; this needs to be done in a Forbid() */
        Forbid();
        if(RexxPort=FindPort(RXSDIR))
          {
          /* We found the port, so put the message to ARexx... */
...
/* This function closes down the ARexx context opened with InitARexx... */
...
void FreeARexx(AREXXCONTEXT RexxContext)
  ...
  if (RexxContext)
    ...
    if (RexxContext->ARexxPort)
      ...
      while (rmsg=GetARexxMsg(RexxContext))
        {
        /* Any messages that still are coming in are "dead"       */
        /* We just set the LASTERROR and reply an error of 100... */
...
AREXXCONTEXT InitARexx(char *AppName,char *Extension)
  ...
  if (RexxContext=AllocMem(sizeof(struct ARexxContext),MEMF_PUBLIC|MEMF_CLEAR))
    ...
    if (RexxContext->RexxSysBase=OpenLibrary("rexxsyslib.library",NULL))
      ...
      for (loop=0;(loop<7)&&(Extension[loop]);loop++) *tmp++=Extension[loop];
      *tmp='\0';

      /* Set up a port name... */

      tmp=RexxContext->PortName;
      for (loop=0;(loop<16)&&(AppName[loop]);loop++) *tmp++=toupper(AppName[loop]);
      *tmp='\0';
...
struct FileHandle *OpenTalkFile(STRPTR filename)
{
	return((struct FileHandle *)Open(filename,MODE_NEWFILE));
}

void CloseTalkFile(STRPTR filename, struct FileHandle *FH)
{
	if (FH)
		Close((BPTR)FH);
	DeleteFile(filename);
...
BOOL IssueRexxCmdNoRet(char *cmd)
{
AREXXCONTEXT RexxStuff;
BOOL retval=FALSE;

	RexxStuff=InitARexx("RXCMD",NULL);
	if (RexxStuff)
	{
		retval=SendARexxMsg(RexxStuff,cmd,TRUE,NULL);
		FreeARexx(RexxStuff);
	}
	return(retval);
}
