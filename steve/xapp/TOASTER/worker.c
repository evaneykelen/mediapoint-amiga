...
extern ULONG XappDoIt(PROCESSINFO *ThisPI, BOOL *sigs, ULONG waitFlags);
...
//			sigs = FALSE;
		...
		if(B_Run)
		{
//printf("addr 1 = %x   %x | %x | %x\n", &sigs, Sig_PtoC, SigR_CtoP, SIGF_ABORT );
			//SigRecvd = XappDoIt(ThisPI, &sigs, Sig_PtoC | SigR_CtoP | SIGF_ABORT);
			XappDoIt(ThisPI, &sigs, Sig_PtoC | SigR_CtoP | SIGF_ABORT);
//printf("sigs=%d, rcvd=%x\n", sigs, SigRecvd);
			B_Run = FALSE;
		}
